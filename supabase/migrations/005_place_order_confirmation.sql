drop function if exists public.place_order(text, text, text, text, text, text, text, public.payment_method, jsonb);
...
begin
  ...
  loop
    ...
    where id = v_variant.product_id and is_active = true and archived_at is null;
  ...
  insert into public.orders (
    order_number, customer_name, customer_phone, customer_email, address, city,
    postal_code, notes, subtotal, shipping_charge, total, payment_method,
    payment_status, order_status
  ) values (
    v_order_number, p_customer_name, p_customer_phone, nullif(p_customer_email, ''),
    p_address, p_city, nullif(p_postal_code, ''), nullif(p_notes, ''),
    v_subtotal, v_shipping, v_total, p_payment_method,
    case when p_payment_method = 'bank_transfer' then 'pending_verification'::public.payment_status else 'pending'::public.payment_status end,
    'pending'
  ...
  loop
    ...
    select url into v_image_url from public.product_images where product_id = v_product.id order by sort_order asc limit 1;
    v_unit_price := coalesce(v_product.sale_price, v_product.regular_price);
    v_line_total := v_unit_price * v_quantity;

    insert into public.order_items (
      order_id, product_id, variant_id, product_name, product_slug, image_url,
      color_name, size, unit_price, quantity, line_total
    ) values (
      v_order_id, v_product.id, v_variant.id, v_product.name, v_product.slug,
      v_image_url, v_variant.color_name, v_variant.size, v_unit_price, v_quantity, v_line_total
    );

    update public.product_variants set stock = stock - v_quantity where id = v_variant.id;
  end loop;

  return jsonb_build_object('orderNumber', v_order_number, 'confirmationToken', v_confirmation_token);
end;
$$;
